...
function savePublication(req, res){
	...
	publication.save((err, publicationStored) => {
		...
		if(!publicationStored) return res.status(404).send({message: 'La publicación no ha sudo guardad.'}); 
...
function getPublication(req,res){
	...
	Publication.findById(publicationId, (err,publication) => {
		...
		if(!publication) return res.status(404).send({message: "Error al devolver publicaciones"});
...
function deletePublication(req, res){
	...
	Publication.find({"user": req.user.sub, '_id': publicationId}).deleteOne((err,publicationRemoved) => {
		...
		if(!publicationRemoved) return res.status(404).send({message: "Error al devolver publicaciones"});
